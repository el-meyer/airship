...
If either we are happy to "blur over" the effect of a simulation input parameter (e.g. check model performance across different priors) or certain input variables are redundant in the sense that they describe the same underlying variation in simulation input parameters (e.g. having a column "scenario" that is a unique identifier for combinations of input variables), they are not chosen as focus variables and hence no default value is chosen.
