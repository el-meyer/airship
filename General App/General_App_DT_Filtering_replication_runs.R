          ...
          tabPanel(
            ...
            fluidRow(
              column(
                ...
                selectInput(
                  "boxplotOutputVar",
                  "Select output variable for boxplot",
                  choices = NULL
                )
              ),
...
server <- function(session, input, output){
  ...
  observe({
    ...
    if(input$checkboxRepvar){
      ...
      showTab("tabs", target = "Boxplot")
    } else {
      hideTab("tabs", target = "Summarized Data")
      hideTab("tabs", target = "Boxplot")
    }
  ...
  # inputvariables
  names_inputsR <- reactive({
    #req(data_choose_default, ind_inputendR)
    nm <- names(data_choose_default())[1:ind_inputendR()]
    if(input$checkboxRepvar){
      nm <- nm[!(nm %in% input$repvar)]
    }
    nm
  ...
  # outputvariables
  names_outputsR <- reactive({
    colnames(data_choose_default())[ind_outputstartR():ncol(data_choose_default())]
  })
  
  data_filteredR <- reactive({
    data_choose_default()[names_inputsR()]
  })
  ...
  #   ,
  #   "ncol(data_choose_default())" = ncol(data_choose_default()),
  #   "colnames(data_choose_default())" = colnames(data_choose_default()),
  #   "head(data_choose_default)" = head(data_choose_default()),
  ...
  output$filteredDT <- DT::renderDT({
    ...
    # if(input$checkboxRepvar){
    #   data_choose_default <- data_full_mean()
    # } else {
    #   data_choose_default <- data_prefiltered()
    # }  
    
    # which column is the last input column?
    # ind_inputend <- isolate(which(colnames(data_choose_default == input$inputend)) 
    
    # display only input columns
    
    # 
    # data_filteredR <- reactive({
    #   data_choose_default()[,1:ind_inputendR()]
    # })
  ...
  nOCR <- reactive({
    ncol(data_choose_default()) - ind_inputendR()
  })
  ...
  df_plot <- reactive({
    ...
    if(length(default_filter) != 0){
      df_plot <- subset(data_choose_default(), eval(parse(text = bedingung)))
    } else {
      df_plot <- data_choose_default()
    }
